...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  // 显示帧计数（灰度条）
  // Note: If your coordinate system is flipped vertically, this bar may appear
  // at the opposite edge. In this package, ShaderSurface defaults to
  // `upSideDown = true`, which flips the widget so a single-edge bar is usually
  // what you want.
  float topBar = step(0.95, uv.y);
