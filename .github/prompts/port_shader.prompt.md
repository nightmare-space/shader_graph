...
---
你现在运行在 VS Code 的 Copilot Chat 里，需要直接修改当前活动的 Shadertoy fragment shader，使其符合 Flutter / Skia / SkSL 环境的兼容要求。请按以下步骤处理（只做必要修改，尽量保持原算法与变量名不变）：
...
- 关于 `iChannel0..3` 的 wrap/filter（必须使用 `SG_TEX0..3`）：
  ...
  - 这些宏会读取 `iChannelWrap` / `iChannelFilter` / `iChannelResolutionN`，在 shader 内部实现 wrap + linear/nearest，从而不依赖 Flutter 后端 `texture()` 的默认 sampler filter。
  ...
  - 性能提示：当 filter=linear 时会做 4 次采样（手动双线性），通常比单次 `texture()` 慢；当 filter=nearest 时开销接近单次 `texture()`。
...
- SkSL/Web 下不支持数组构造器/初始化器，包含但不限于：
  - `const int[] arr = int[](...)`
